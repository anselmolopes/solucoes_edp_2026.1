...
\analise{
	\textbf{Análise do Enunciado:} Esta questão foi estruturada como um roteiro de construção analítica. O objetivo não é apenas encontrar a solução final, mas derivar a fórmula de d'Alembert para a equação da onda através de etapas intermediárias. A lógica consiste em utilizar a teoria de equações de transporte (de 1\f ordem) para resolver a equação da onda (de 2\m ordem). Assim, cada item, de (a) a (e), atua como um lema necessário para a conclusão final no item (f). As hipóteses centrais são a regularidade $u \in C^2$ no domínio $\mathbb{R} \times [0, \infty)$ e as condições iniciais clássicas $u(x,0) = g(x)$ e $u_t(x,0) = h(x)$.
	
	\par\textbf{Fundamentação Teórica:} A base para a resolução é o Método das Características (Evans, Cap. 2.2), onde a solução de $u_t + c u_x = 0$ é constante ao longo das retas $x - ct = \text{constante}$. Para a parte não homogênea, aplicamos o Princípio de Duhamel (Evans, Cap. 2.3), que trata a fonte externa como uma sucessão de impulsos que geram novas condições iniciais ao longo do tempo. Por fim, utilizamos a fatoração do operador de onda $\partial_{tt} - \partial_{xx} = (\partial_t - \partial_x)(\partial_t + \partial_x)$, o que justifica a introdução de variáveis auxiliares para reduzir a ordem da EDP.
	
	\par\textbf{Estratégias:} O plano de ataque consiste em resolver a equação de transporte homogênea no item (a) e a não homogênea no item (b). No item (d), definiremos a variável auxiliar $v = u_t - u_x$ para transformar a equação da onda em uma de transporte. Em seguida, interpretaremos $u$ no item (e) como um problema de transporte não homogêneo. Finalmente, aplicaremos o Princípio da Superposição no item (f), somando a solução homogênea e a particular para chegar à fórmula de d'Alembert.
}
...
\solucao{
	...
	$$ \frac{d}{dt} v(x(t), t) = v_t + \frac{dx}{dt}v_x = v_t + v_x $$
	...
	\textbf{(b)} Agora analisamos $w_t - w_x = f(x, t)$ com $w(x,0) = 0$. As características são $\frac{dx}{ds} = -1$. Para atingir o ponto $(x, t)$, a trajetória é $x(s) = x + t - s$. Calculando a derivada de $w$ ao longo dessa curva:
	$$ \frac{d}{ds} w(x + t - s, s) = w_s - w_x $$
	Pela equação, $w_s - w_x = f(x + t - s, s)$. Integrando de $s=0$ até $s=t$:
	$$ \int_0^t \frac{d}{ds} w(x + t - s, s) ds = \int_0^t f(x + t - s, s) ds $$
	$$ w(x, t) - w(x+t, 0) = \int_0^t f(x + t - s, s) ds $$
	Como $w(x,0) = 0$, temos $w(x, t) = \int_0^t f(x + t - s, s) ds$. Este resultado é a aplicação direta do \textbf{Princípio de Duhamel}, onde a solução do problema não homogêneo é construída integrando a fonte $f$ como se fossem sucessivos dados iniciais.
	
	\textbf{(c)} Usamos o resultado de (b) com a fonte $f(x, t) = g(x - t)$:
	...
	$$ w(x, t) = \int_{x+t}^{x-t} g(\sigma) \left( -\frac{1}{2} \right) d\sigma = \frac{1}{2} \int_{x-t}^{x+t} g(\sigma) d\sigma $$
	Se $G$ é a primitiva de $g$, temos $w(x, t) = \frac{1}{2} [G(x + t) - G(x - t)]$.
	...
	\vspace{0.2cm}
	Agora, observemos que a equação $v_t + v_x = 0$ com dado inicial $v(x,0) = h(x) - g'(x)$ é idêntica à estrutura resolvida no item (a). Portanto, a solução para $v$ é a translação do seu dado inicial:
	$$ v(x, t) = h(x - t) - g'(x - t) $$
	
	\textbf{(e)} Pela definição de $v$ no item (d), temos:
	$$ u_t - u_x = v(x, t) = h(x - t) - g'(x - t) $$
	com $u(x, 0) = g(x)$. Esta é a equação de transporte não homogênea do item (b) com fonte $f(x, t) = h(x - t) - g'(x - t)$.
	...
	Para $u_p$, resolvemos $u_t - u_x = f$ com $u(x,0) = 0$. Usando o resultado de (b) com $f(x, s) = h(x - s) - g'(x - s)$:
...
\sintese{
	A resolução desta questão demonstra que a equação da onda é a superposição de dois fluxos de transporte em direções opostas. A "Grande Montagem" do raciocínio pode ser resumida no seguinte fluxo:
	...
	O item (a) fornece a técnica de solução para a direita, usada no item (d) para descobrir a variável auxiliar $v$. Esta variável $v$ torna-se a fonte de um transporte para a esquerda no item (e). Finalmente, no item (f), unimos a solução homogênea (do transporte para a esquerda) com a solução particular (via Duhamel, baseada no item b) para reconstruir a solução completa de d'Alembert.
}
