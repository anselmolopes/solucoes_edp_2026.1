...
\analise{
	\textbf{Análise do Enunciado}: O problema apresenta uma identidade integral que envolve a energia do gradiente, um termo fonte $f$ e uma integral de contorno ponderada por um campo vetorial $a$. O objetivo é extrair a equação diferencial governante no interior do domínio $U$ e a condição de contorno na fronteira $\partial U$. 
	
	\textbf{Fundamentação Teórica}: 
	\begin{itemize}
		\item \textbf{Primeira Identidade de Green}: Para $u \in H^2(U)$ e $v \in C^1(\overline{U})$, temos 
		$$\int_U \nabla u \cdot \nabla v \, dx = - \int_U (\Delta u) v \, dx + \int_{\partial U} v \frac{\partial u}{\partial \nu} d\sigma.$$
		\item \textbf{Definição de Derivada Normal}: A derivada normal $\frac{\partial u}{\partial \nu}$ é a projeção do gradiente sobre o vetor normal exterior: $\frac{\partial u}{\partial \nu} = \nabla u \cdot \nu = \sum_{j=1}^n \nu_j \frac{\partial u}{\partial x_j}$.
		\item \textbf{Lema Fundamental do Cálculo de Variações}: Se $\int_U g \phi \, dx = 0$ para toda $\phi \in C_0^\infty(U)$, então $g = 0$ quase sempre em $U$.
	\end{itemize}
	
	\textbf{Estratégias}:
	\begin{enumerate}
		\item Substituiremos a integral $\int_U \nabla u \cdot \nabla v \, dx$ utilizando a Primeira Identidade de Green.
		\item Reorganizaremos a equação para agrupar todos os termos integrais de volume de um lado e os de superfície do outro.
		\item Primeiro, escolheremos funções teste $v$ com suporte compacto ($v \in C_0^\infty(U)$) para anular a fronteira e identificar $-\Delta u = f$.
		\item Em seguida, com a equação do interior satisfeita, utilizaremos a liberdade de escolha de $v$ na fronteira para identificar a condição de contorno.
	\end{enumerate}
}
...
\solucao{\par
	...
	$$\int_U (-\Delta u - f) v \, dx + \int_{\partial U} v \left( \frac{\partial u}{\partial \nu} - \sum_{j=1}^n a_j T \left( \frac{\partial u}{\partial x_j} \right) \right) d\sigma = 0 \quad (\ast)$$
	
	\textbf{1. Identificação da Equação no Interior}:
	Para isolar o comportamento em $U$, escolhemos funções teste $v \in C_0^\infty(U)$. Por definição, tais funções anulam-se na fronteira $\partial U$, logo o termo de superfície em $(\ast)$ desaparece:
	...
	\textbf{2. Identificação da Condição de Contorno}:
	Agora que provamos que $-\Delta u - f = 0$ q.s. em $U$, o termo de volume na equação $(\ast)$ é nulo para qualquer $v \in C^1(\overline{U})$. A identidade reduz-se a:
	...
	Como a igualdade deve valer para qualquer função $v$ na fronteira, o termo entre parênteses deve ser nulo quase sempre em $\partial U$:
...
\sintese{
	...
	$$\boxed{\text{Identidade de Green} \to \text{Teste em } C_0^\infty(U) \to \text{Equação Forte} \to \text{Isolamento da Fronteira} \to \text{Condição de Contorno}}.$$
}
