...
\solucao{
	...
	Definimos a sequência de funções $f_n(t) = \chi_{[a, x_n]}(t) v(t)$. Como $x_n \to x$, temos pontualmente que $\chi_{[a, x_n]}(t) \to \chi_{[a, x]}(t)$ para todo $t \neq x$. Como o ponto $\{x\}$ tem medida de Lebesgue nula, temos $f_n(t) \to \chi_{[a, x]}(t) v(t)$ quase sempre (q.s.) em $I$.
	
	Além disso, para todo $n \in \mathbb{N}$, vale a estimativa:
	$$ |f_n(t)| = |\chi_{[a, x_n]}(t) v(t)| \le |v(t)| \quad \text{q.s. em } I $$
	Como $v \in L^1(I)$, a função $|v|$ atua como uma limitante integrável. Pelo \textbf{Teorema da Convergência Dominada de Lebesgue}, podemos passar o limite para dentro da integral:
	...
	Lendo essa mesma desigualdade com foco na variável $t$, vemos que a restrição sobre $t$ é $s \le t \le b$, ou seja, $t \in [s, b]$. Logo, $\chi_{[a,t]}(s) = \chi_{[s,b]}(t)$. Substituindo na integral, ficamos com:
	...
	Dado que o suporte de $\psi$ é compacto e contido no interior de $(a,b)$, temos obrigatoriamente $\psi(b) = 0$. Logo:
	$$ \int_s^b \psi'(t) dt = 0 - \psi(s) = -\psi(s) $$
	Substituindo de volta na integral iterada:
	$$ \int_a^b w(t) \psi'(t) dt = \int_a^b v(s) \bigl( -\psi(s) \bigr) ds = - \int_a^b v(s) \psi(s) ds $$
	Trocando a variável muda $s$ por $t$, obtemos:
	$$ \boxed{ \int_I w(t) \psi'(t) dt = - \int_I v(t) \psi(t) dt, \quad \forall \psi \in C_c^\infty(I) } $$
	Isto demonstra que a derivada fraca de $w$ é exatamente $v$.
}
...
\aprofundamento{
	Este exercício estabelece um resultado central para o estudo de espaços de Sobolev em dimensão $n=1$: o espaço $W^{1,1}(I)$ coincide com o espaço das funções absolutamente contínuas (AC) cujas derivadas fracas pertencem a $L^1(I)$.
	
	Enquanto em dimensões superiores ($n > 1$) as funções de Sobolev podem ser descontínuas em pontos ou superfícies (dependendo do expoente $p$), em $n=1$, qualquer função com derivada fracamente integrável admite um representante contínuo. Este facto é a base matemática por trás da imersão contínua $W^{1,p}(I) \hookrightarrow C^{0, 1 - 1/p}(\bar{I})$ dada pelo Teorema de Morrey.
}
\end{document}