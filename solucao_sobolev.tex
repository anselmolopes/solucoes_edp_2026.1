...
\solucao{\par
	...
	Aplicando a \textbf{Desigualdade de Minkowski} (no sentido de que a norma de uma soma é menor ou igual à soma das normas) e a cota $\|\nabla \gamma\|_\infty$, estimamos a magnitude do gradiente $|\nabla v| = \sqrt{|\nabla_z v|^2 + |\partial_{x_n} v|^2} \le |\nabla_z v| + |\partial_{x_n} v|$:
	\begin{align*}
	...
	$$|\nabla v(z, x_n)| \le (3 + 6\|\nabla \gamma\|_\infty) |\nabla u(P_1)| + (6 + 6\|\nabla \gamma\|_\infty) |\nabla u(P_2)|$$
	Elevando à potência $p$ e utilizando novamente a desigualdade de convexidade $|a+b|^p \le 2^{p-1}(|a|^p + |b|^p)$:
...
%%%% QUESTÃO 119 %%%%
\questao{
	Seja $U$ um aberto e ${V_k}_{k \in \mathbb{N}}$ uma cobertura de $U$ localmente finita, ou seja, $U = \bigcup_{k=1}^\infty V_k$, $V_k \subset U$, e para cada compacto $K \subset U$, devemos ter $K \cap V_k = \emptyset$, para todo $k$, exceto em uma quantidade finita de $k$.
...
\analise{
	\textbf{Análise do Enunciado}: A primeira parte exige a generalização da Primeira Identidade de Green para funções em espaços de Sobolev com expoentes conjugados $p$ e $q$. A segunda parte solicita a prova de uma desigualdade de energia para funções que se anulam na fronteira ($H_0^1$), relacionando a norma do gradiente com a norma da função e do seu Laplaciano.
	
	\textbf{Fundamentação Teórica}: A prova do item (a) baseia-se na densidade de $C^\infty(\overline{U})$ em $W^{1,p}(U)$ e $W^{2,q}(U)$. Para funções suaves, a identidade é trivial via Teorema de Gauss; a extensão para Sobolev ocorre por limite. O operador de traço $T$ garante que as integrais na fronteira sejam bem definidas. Utilizaremos a continuidade do operador traço $T: W^{1,p}(U) \to L^p(\partial U)$.
	
	\textbf{Estratégias}:
	\begin{enumerate}
		\item \textbf{Aproximação por Funções Suaves}: Estabelecer a identidade para $u_k, v_k \in C^\infty(\overline{U})$.
		\item \textbf{Análise de Convergência no Domínio}: Decompor a diferença das integrais de volume e aplicar Hölder para provar que os termos convergem para as integrais de $u$ e $v$.
		\item \textbf{Análise de Convergência na Fronteira}: Utilizar a continuidade do operador de traço e a desigualdade de Hölder no espaço $L^p(\partial U) \times L^q(\partial U)$ para provar a convergência do termo de superfície.
		\item \textbf{Aplicação de Cauchy-Schwarz}: No item (b), anular o traço por $u \in H_0^1$ e aplicar a desigualdade de Cauchy-Schwarz.
	\end{enumerate}
}

\solucao{
	\textbf{(a) Demonstração da Identidade de Green}:
	Sejam $u \in W^{1,p}(U)$ e $v \in W^{2,q}(U)$. Pela densidade de $C^\infty(\overline{U})$ em $W^{1,p}(U)$ e $W^{2,q}(U)$, existem sequências $\{u_k\} \subset C^\infty(\overline{U})$ e $\{v_k\} \subset C^\infty(\overline{U})$ tais que $u_k \to u$ em $W^{1,p}(U)$ e $v_k \to v$ em $W^{2,q}(U)$. Para cada $k$, a identidade de Green clássica é válida:
	...
	\textbf{1. Convergência do termo $\int_U u \Delta v$}:
	...
	\end{align*}
	Aplicando a \textbf{Desigualdade de Hölder} para $(p, q)$:
	\begin{align*}
		\text{Erro}_1 &\le \|u_k - u\|_{L^p(U)} \|\Delta v_k\|_{L^q(U)} + \|u\|_{L^p(U)} \|\Delta v_k - \Delta v\|_{L^q(U)}
	\end{align*}
	Como $\Delta v_k \to \Delta v$ em $L^q(U)$, a sequência $\|\Delta v_k\|_{L^q}$ é limitada por uma constante $C$.
	
	Assim, $\text{Erro}_1 \le \|u_k - u\|_{L^p} \cdot C + \|u\|_{L^p} \|\Delta v_k - \Delta v\|_{L^q}$, o que tende a zero quando $k \to \infty$.
	...
	Seja $g_k = \sum_{j=1}^n \nu_j T \frac{\partial v_k}{\partial x_j}$ e $g = \sum_{j=1}^n \nu_j T \frac{\partial v}{\partial x_j}$. Como $v_k \to v$ em $W^{2,q}(U)$, a continuidade do operador traço $T: W^{1,q}(U) \to L^q(\partial U)$ aplicado às derivadas $\partial_j v$ garante que $g_k \to g$ em $L^q(\partial U)$. Analisamos a diferença:
	\begin{align*}
	...
	\end{align*}
	Aplicando a \textbf{Desigualdade de Hölder} em $\partial U$:
	\begin{align*}
		\text{Erro}_{\text{fronteira}} &\le \|T u_k - T u\|_{L^p(\partial U)} \|g_k\|_{L^q(\partial U)} + \|T u\|_{L^p(\partial U)} \|g_k - g\|_{L^q(\partial U)}
	\end{align*}
	Pela continuidade do operador de traço $T_p$, temos $\|T u_k - T u\|_{L^p(\partial U)} \le C \|u_k - u\|_{W^{1,p}(U)} \to 0$. Como $\|g_k\|_{L^q(\partial U)}$ é limitada e $\|g_k - g\|_{L^q(\partial U)} \to 0$, o erro de fronteira tende a zero.
	...
	Seja $u \in H^2(U) \cap H_0^1(U)$. Aplicamos a identidade do item (a) com $p=q=2$ e $v=u$. Como $u \in H_0^1(U)$, o traço $T u = 0$ quase sempre em $\partial U$. A integral de superfície anula-se:
	$$\int_U u \Delta u \, dx + \int_U |\nabla u|^2 \, dx = 0 \implies \int_U |\nabla u|^2 \, dx = - \int_U u \Delta u \, dx$$
	Tomando o módulo e aplicando a \textbf{Desigualdade de Cauchy-Schwarz} em $L^2(U)$:
	\begin{align*}
		\int_U |\nabla u|^2 \, dx = \left| \int_U u \Delta u \, dx \right| \le \int_U |u \Delta u| \, dx \le \left( \int_U u^2 \, dx \right)^{\frac{1}{2}} \left( \int_U (\Delta u)^2 \, dx \right)^{\frac{1}{2}}
	\end{align*}
...
\sintese{
	...
	O fluxo lógico foi:
	$$\boxed{\text{Identidade Suave} \to \text{Decomposição do Erro} \to \text{Hölder } (L^p, L^q) \to \text{Traço Contínuo} \to \text{Limite Sobolev}}$$
}
...
\analise{
	\textbf{Análise do Enunciado}: O objetivo é provar que a "colagem" de duas funções de Sobolev, cujos traços coincidem na interface comum $\Gamma$, resulta em uma função que mantém a regularidade $W^{1,p}$ no domínio unificado. A tese central é que a igualdade dos traços anula o "salto" potencial na interface, permitindo que a derivada fraca seja a união das derivadas locais.
	
	\textbf{Fundamentação Teórica}: A prova baseia-se na definição de derivada fraca via integração por partes. Se a função fosse descontínua em $\Gamma$, a integração por partes produziria um termo de superfície (uma medida concentrada na interface). A hipótese $T_p v = T_p w$ garante que esse termo se anule. Referência: \textbf{Brezis}, Capítulo 8.
...
\solucao{
	...
	\textbf{3. Conclusão}:
	Restou provado que $\int_U u \partial_j \phi \, dx = - \int_U g_j \phi \, dx$ para toda $\phi \in C_c^\infty(U)$. Assim, a derivada fraca de $u$ existe e é dada por $\nabla u = \nabla v$ em $U_1$ e $\nabla u = \nabla w$ em $U_2$. Como $\nabla v \in L^p(U_1)$ e $\nabla w \in L^p(U_2)$, temos $\nabla u \in L^p(U)$. Portanto, $u \in W^{1,p}(U)$.
}
...
\setcounter{numquestao}{131} % Define o valor atual como 131
