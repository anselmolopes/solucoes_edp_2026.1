...
\newbool{showsolucaoalt} 
\setbool{showsolucaoalt}{true}
...
\newbool{showresultadoextra}
\setbool{showresultadoextra}{true}
...
\newcommand{\questao}[1]{%
	...
	\setcounter{equation}{0}% <--- GARANTE QUE A QUESTÃO ZERE AS EQUAÇÕES GERAIS
	%
...
\newcommand{\solucao}[1]{%
	\ifbool{showcurrentQ}{%
		\setcounter{equation}{0}% <--- ADICIONE ESTA LINHA AQUI
		\par\vspace{5pt}
...
\newcommand{\resultadoextra}[2]{%
	\ifbool{showcurrentQ}{% <--- Se a questão atual estiver oculta, ele também some
		\ifbool{showresultadoextra}{% <--- Checa a sua chave global de exibição
